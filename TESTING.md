...
`vitest.config.ts` の `coverage.thresholds` で**CIによって強制**されます。下限を
下回ると `npm run test:coverage`（およびCIのtestジョブ）が失敗します。

### 強制されている下限（2026-07 時点の実測: Stmts 81% / Branch 80% / Func 77% / Lines 81%）
- **ステートメント**: 78%以上
- **ライン**: 78%以上
- **ブランチ**: 75%以上
- **関数**: 74%以上

> カバレッジが上がったら閾値も引き上げること（`vitest.config.ts`）。
...
※ **`api/`（Vercel Edge関数）はカバレッジ対象に含まれます**（セッション署名・
レート制限・サーキットブレーカー等のセキュリティ中核を計測するため）。
